...
def test_read_login(client, headers, auth_uid):
...
def test_read_user(client, headers):
...
def test_patch_user(client, headers):
...
def test_create_extract_and_process_ergImage(client, headers):
...
def test_create_workout(client, headers):
...
def test_read_workout(client, headers):
    resp = client.get("/workout", headers=headers)
    assert "workouts" in resp.json()["body"]


def test_delete_workout(client, headers):
    resp = client.get("/workout", headers=headers)
...
def test_write_team(client, headers):
...
def test_read_team(client, headers):
...
def test_create_feedback(client, headers):
